...
final class MessageCourier {
    ...
    private static let scriptQueue = DispatchQueue(
        label: "cove.courier.script", qos: .userInitiated
    )
    ...
    func stage(
        ...
        for length in stride(from: min(3, tokens.count), through: 1, by: -1) {
            ...
            case .many(let names) where ambiguous == nil:
                ambiguous = names
            case .denied:
                return "Cove can't read Contacts. System Settings, Privacy and Security, Contacts. Or text the number itself."
            case .unasked:
                return "macOS is asking to let Cove see Contacts. Click Allow, then say it again."
            case .failed:
    ...
    func confirmSend() async -> String {
        ...
        case -1744:
            primeMessagesGrant()
            return "macOS is asking to let Cove use Messages. Click Allow, then say send."
        case -1743:
            return "macOS blocked Cove from Messages. System Settings, Privacy and Security, Automation, then say send again."
        default:
            break
        }
