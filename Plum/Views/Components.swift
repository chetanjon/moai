...
private struct PlumCard: ViewModifier {
    var radius: CGFloat
...
extension View {
    func plumCard(radius: CGFloat = Theme.Radius.card) -> some View {
        modifier(PlumCard(radius: radius))
    }
}

/// The field treatment: brighter fill than a card, hairline stroke
/// that warms to the accent while the field is in use.
private struct PlumField: ViewModifier {
    var active: Bool
...
extension View {
    func plumField(active: Bool = false) -> some View {
        modifier(PlumField(active: active))
    }
}
...
/// Three dots doing a gentle wave while Plum works.
