...
private struct CoveCard: ViewModifier {
    var radius: CGFloat
...
extension View {
    func coveCard(radius: CGFloat = Theme.Radius.card) -> some View {
        modifier(CoveCard(radius: radius))
    }
}

/// The field treatment: brighter fill than a card, hairline stroke
/// that warms to the accent while the field is in use.
private struct CoveField: ViewModifier {
    var active: Bool
...
extension View {
    func coveField(active: Bool = false) -> some View {
        modifier(CoveField(active: active))
    }
}
...
/// Three dots doing a gentle wave while Cove works.
