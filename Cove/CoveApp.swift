...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    /// Copies the old com.cj.moai defaults domain wholesale, then
    /// re-homes the keys that carried the old name, so notes, focus
    /// history, and every setting survive the rename. Runs once; a
    /// fresh install finds nothing and moves on. Existing values are
    /// never overwritten.
    private func migrateFromMoai() {
        let defaults = UserDefaults.standard
        guard !defaults.bool(forKey: "cove.migratedFromMoai"),
              let old = defaults.persistentDomain(forName: "com.cj.moai")
        else { return }
        ...
        for key in ["onboarded", "lastMusicApp", "lastUpdateNudge",
            ...
            if let value = old["moai." + key],
               defaults.object(forKey: "cove." + key) == nil {
                defaults.set(value, forKey: "cove." + key)
            }
        }
        defaults.set(true, forKey: "cove.migratedFromMoai")
    }
}
